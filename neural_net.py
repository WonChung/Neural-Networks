...
class SigmoidNode(Node):
    ...
    def compute_activation(self):
        ...
        for i in range(len(self.in_edges)):
            x += self.in_edges[i].weight*self.in_edges[i].source.activation
    ...
    def compute_hidden_delta(self):
        ...
        weightedSum = 0
        for i in range(len(self.out_edges)):
            weightedSum += self.out_edges[i].old_weight*self.out_edges[i].dest.delta
        self.delta = self.activation * (1 - self.activation) * weightedSum
    ...
    def update_weights(self):
        ...
        for i in range(len(self.in_edges)):
            new_weight = self.in_edges[i].weight + self.learning_rate*self.delta*self.in_edges[i].source.activation
            self.in_edges[i].change_weight(new_weight)
...
class Network(object):
    ...
    def predict(self, input_vector):
        ...
        for i in range(1, len(self.layers)):
            for j in range(len(self.layers[i])):
                self.layers[i][j].compute_activation()
        predictions = []
        for i in range(len(self.layers[-1])):
            predictions.append(self.layers[-1][i].activation)
        return predictions
    ...
    def train(self, data_set, epochs, verbose=False, random_seed=None):
        ...
        for i in range(epochs):
            ...
            for j in range(len(data_set)):
                input_vector = data_set[j][0]
                target_vector = data_set[j][1]
                predictions = self.predict(input_vector)
                data_set_targets.append(target_vector)
                data_set_predictions.append(predictions)
                self.backpropagation(target_vector)
            if(allclose(data_set_predictions, data_set_targets, rtol=self.converge, equal_nan=True)):
                if(verbose==True):
                    print("Converged on Epoch:", i)
                converged = True
                break
        if(converged==False and verbose==True):
    ...
    def test(self, data_set):
        ...
        for i in range(len(data_set)):
            input_vector = data_set[i][0]
            target_vector = data_set[i][1]
            predictions = self.predict(input_vector)
            if(allclose(predictions, target_vector, rtol=self.converge, equal_nan=True)):
                accuracy += 1
...
def main():
    ...
    nn = Network(2, 1, [2], random_seed=5)
    nn.train(data_set, 10000, verbose=True, random_seed=5)
    print("\nresulting", nn, "\n")
